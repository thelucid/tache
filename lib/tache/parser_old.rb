  ...
  def parse(source = '', tags = nil)
    ...
    until scanner.eos?
      ...
      text.lines.each do |line|
        tokens << ['text', line]
        tokens << ['indent'] if line.end_with?("\n")
      end
      tokens.pop if tokens.last && tokens.last[0] == 'indent' && scanner.eos?
      ...
      tokens << ['indent'] unless tail.empty? || scanner.eos?
    end
